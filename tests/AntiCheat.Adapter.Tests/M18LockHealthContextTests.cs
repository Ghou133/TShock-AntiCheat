...
public sealed class M18LockHealthContextTests
{
    ...
    private (SessionSnapshot? Session, TSPlayer? Player) Lookup(int slot) => slot == Slot
        ? (new(session, boundAccountId, false, DateTimeOffset.UtcNow), actor)
        : (null, null);
    ...
    [Test]
    public void ChangedAccountOrMissingGroupCannotInheritServiceKickPairs()
    {
        Assert.That(Pair().PairCount, Is.EqualTo(1));
        var originalAccount = actor.Account;
        var originalGroup = actor.Group;
        actor.Account = new UserAccount { ID = 19020, Name = "other-account" };
        try
        {
            Assert.That(Pair().PairMatched, Is.False);
            Assert.That(Pair().Kick, Is.False);
            Assert.That(context.Healthy, Is.True);
        }
        finally { actor.Account = originalAccount; }
        ...
        finally { actor.Group = originalGroup; }
        Assert.That(Pair().PairCount, Is.EqualTo(1));
    }
