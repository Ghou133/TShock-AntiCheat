...
public sealed partial class M6NpcStrikeTests
{
    ...
    public async Task M14RWorldAlignment_FirstPublicationCancelsRevokesAndPersistsOnlyRealAccount(bool earlierCancelled)
    {
        var innocent = engine.OpenSession(22)!.Value; engine.Authenticate(innocent, 1462);
        int later = 0;
        void Observe(GetDataEventArgs args) { if (!args.Handled) later++; }
        ServerApi.Hooks.NetGetData.Register(plugin, Observe, 0);
        try
        {
            var args = M2ContractsTests.Packet((PacketTypes)57, new byte[3], Slot); args.Handled = earlierCancelled;
            ...
            var result = M14RWorldAlignmentPacketReader.Evaluate(new(0, 0, 0), session,
                ServerTShock.Players[Slot], TargetRuntime.Fingerprint, true);
            var existing = engine.ObserveBusiness(new(session, 57, result,
            ...
            Assert.That(existing.Incident.Evidence.RuleId, Is.EqualTo(M14RWorldAlignmentRules.RuleId));
    ...
    [Test]
    public void M14RWorldAlignment_MalformedOnlyBlocksAndLaterLegalRequestRemainsPossible()
    {
        foreach (int length in new[] { 0, 2, 4 })
        {
            var args = M2ContractsTests.Packet((PacketTypes)57, new byte[length], Slot);
