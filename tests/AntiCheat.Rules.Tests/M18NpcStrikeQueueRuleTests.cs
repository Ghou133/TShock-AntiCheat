...
public sealed class M18NpcStrikeQueueRuleTests
{
    ...
    private static M18NpcStrikeObservation Observation(
            ...
            generationMatches, clientOrigin, attributionComplete, legalExceptionsExcluded)
        {
            ...
            ActorX = targetX,
            ActorY = targetY,
            ActorPositionSnapshotComplete = targetPositionSnapshotComplete || targetX != 0f || targetY != 0f,
    ...
    [Test]
    public void SourceShapedSequenceIsObservedBeforeThe64HitBudget()
    {
        var queue = new M18NpcStrikeQueue(Options with
        {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(second.Action, Is.EqualTo(ControlAction.Unknown));
            Assert.That(second.ButcherPatternDetected, Is.True);
            Assert.That(second.IsButcherPatternBlock, Is.False);
            Assert.That(second.ButcherCrossTargetContinuation, Is.True);
            Assert.That(second.ButcherCompletedTargetSequences, Is.EqualTo(1));
            Assert.That(second.Reason, Is.EqualTo("npc-strike-queue-record-only"));
        });
    }

    [Test]
    public void LowDamageCrossTargetButcherSequenceIsObservedWithoutAccountSanction()
    {
        var queue = new M18NpcStrikeQueue(Options with
        {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(first.Action, Is.EqualTo(ControlAction.Unknown));
            Assert.That(second.Action, Is.EqualTo(ControlAction.Unknown));
            Assert.That(second.Verdict, Is.EqualTo(Verdict.Unknown));
            Assert.That(second.ButcherPatternDetected, Is.True);
            Assert.That(second.IsButcherPatternBlock, Is.False);
            Assert.That(second.IsButcherSequenceSanctionCandidate, Is.False);
            Assert.That(second.PermanentSanctionCandidate, Is.False);
            Assert.That(second.ButcherCrossTargetContinuation, Is.True);
            Assert.That(second.ButcherControlJumpDetected, Is.True);
            Assert.That(second.ButcherMatchingTargetDeathSequences, Is.EqualTo(1));
            Assert.That(second.ButcherCompletedTargetSequences, Is.EqualTo(1));
            ...
            Assert.That(rule.Facts["strikeQueueClassification"], Is.EqualTo("ordinary-butcher-sequence-observed"));
            Assert.That(rule.Facts["strikeQueueActionContract"], Is.EqualTo("record-only"));
        });
    }

    [Test]
    public void OrdinaryMovingCrossTargetKillAcrossTicksDoesNotTriggerSequenceBlock()
    {
        var queue = new M18NpcStrikeQueue(Options with
        {
            EnablePermanentSanctions = true,
        ...
        queue.ObservePostNative(1, new M18NpcStrikePostNativeObservation(
            ...
            first.Stage, true, first.ButcherPositionContextComplete,
            first.ButcherPreControlAtTarget, first.ButcherControlJumpDetected,
            true, true, true, true, true, true), first);
        queue.ObservePlayerControls(2, Control(100f, 100f) with
        { ServerActorX = 500f, ServerActorY = 500f });
        queue.ObservePlayerControls(3, Control(700f, 700f));
        var second = queue.Observe(3, Observation(targetSlot: 12, targetGeneration: 4,
            targetX: 700f, targetY: 700f, wireDamage: 1, receiverDamage: 1));
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(second.ButcherCrossTargetContinuation, Is.True);
            Assert.That(second.ButcherControlJumpDetected, Is.True);
            Assert.That(second.ButcherPatternDetected, Is.False);
            Assert.That(second.Action, Is.EqualTo(ControlAction.Unknown));
            Assert.That(second.PermanentSanctionCandidate, Is.False);
    ...
    [Test]
    public void CancelledButcherTailRestoreCannotCloseASanctionableSegment()
    {
        var queue = new M18NpcStrikeQueue(Options with
        {
            EnablePermanentSanctions = true,
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(second.ButcherCompletedTargetSequences, Is.Zero);
            Assert.That(second.IsButcherSequenceSanctionCandidate, Is.False);
        });
    }
    ...
    [Test]
    public void RepeatedStationarySameTargetSignatureDoesNotTriggerButcherStopLoss()
    {
        var queue = new M18NpcStrikeQueue(Options with
        {
            PerTargetLowDamageLimit = 64,
        ...
        queue.AdvanceWorld(Actor.WorldEpoch);
        queue.ObservePlayerControls(1, Control(500f, 500f));
        queue.ObservePlayerControls(1, Control(500f, 500f) with
        { ServerActorX = 500f, ServerActorY = 500f });
        var first = queue.Observe(1, Observation(targetX: 500f, targetY: 500f,
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(tail.Action, Is.EqualTo(ControlAction.Unknown));
            Assert.That(tail.IsButcherPatternBlock, Is.False);
            Assert.That(tail.ButcherCurrentTargetStrikes, Is.EqualTo(3));
            Assert.That(result.Facts["strikeQueueClassification"], Is.EqualTo("high-damage-record"));
            Assert.That(result.Facts["strikeQueueActionContract"], Is.EqualTo("record-only"));
            Assert.That(result.PredicateSatisfied, Is.False);
            Assert.That(result.PrerequisitesComplete, Is.False);
        });
    }
    ...
    [Test]
    public void StationaryCrossTargetHitsRemainBelowButcherStopLoss()
    {
        var queue = new M18NpcStrikeQueue(Options with
        {
            PerTargetLowDamageLimit = 64,
        ...
        queue.AdvanceWorld(Actor.WorldEpoch);
        queue.ObservePlayerControls(1, Control(500f, 500f));
        queue.ObservePlayerControls(2, Control(500f, 500f) with
        { ServerActorX = 500f, ServerActorY = 500f });
        var first = queue.Observe(3, Observation(targetSlot: 11, targetGeneration: 3,
            targetX: 500f, targetY: 500f, wireDamage: 10, receiverDamage: 10));
        queue.ObservePlayerControls(4, Control(500f, 500f) with
        { ServerActorX = 500f, ServerActorY = 500f });
        queue.ObservePlayerControls(5, Control(500f, 500f) with
        { ServerActorX = 500f, ServerActorY = 500f });
        var second = queue.Observe(6, Observation(targetSlot: 12, targetGeneration: 4,
            targetX: 510f, targetY: 510f, wireDamage: 10, receiverDamage: 10) with
        { ActorX = 500f, ActorY = 500f });
    ...
    [Test]
    public void CancelledRawControlAttemptIsObservedWithoutBecomingAcceptedMovement()
    {
        var queue = new M18NpcStrikeQueue(Options with
        { PerTargetLowDamageLimit = 64, PerSessionLowDamageLimit = 192 });
        queue.AdvanceWorld(Actor.WorldEpoch);
        queue.ObservePlayerControls(1, Control(500f, 500f));
        queue.ObservePlayerControls(2, Control(500f, 500f) with
        { ServerActorX = 500f, ServerActorY = 500f });
        queue.Observe(3, Observation(targetX: 500f, targetY: 500f,
            wireDamage: 10, receiverDamage: 10));
        queue.ObservePlayerControls(4, Control(700f, 700f, alreadyCancelled: true));
        var later = queue.Observe(5, Observation(targetSlot: 12, targetGeneration: 4,
            targetX: 700f, targetY: 700f, wireDamage: 10, receiverDamage: 10) with
        { ActorX = 500f, ActorY = 500f });

        Assert.Multiple(() =>
        {
            Assert.That(later.ButcherRawCancelledControlAttemptObserved, Is.True);
            Assert.That(later.ButcherPreControlAtTarget, Is.False);
            Assert.That(later.ButcherControlJumpDetected, Is.False);
            Assert.That(later.Action, Is.EqualTo(ControlAction.Unknown));
            Assert.That(later.IsButcherSequenceSanctionCandidate, Is.False);
        });
    }
    ...
    [Test]
    public void HighOverkillCrossTargetSequenceRemainsObservationAfterNativeDeath()
    {
        var queue = new M18NpcStrikeQueue(Options with
        {
            EnablePermanentSanctions = true,
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(completed.PermanentSanctionCandidate, Is.False);
            Assert.That(second.ButcherCrossTargetContinuation, Is.True);
            Assert.That(second.ButcherCompletedTargetDeathSequences, Is.EqualTo(1));
            Assert.That(second.IsButcherSequenceSanctionCandidate, Is.False);
            Assert.That(second.ButcherPatternDetected, Is.True);
            ...
            Assert.That(postRule.Verdict, Is.EqualTo(Verdict.Unknown));
            Assert.That(postRule.PredicateSatisfied, Is.False);
        });
    }
