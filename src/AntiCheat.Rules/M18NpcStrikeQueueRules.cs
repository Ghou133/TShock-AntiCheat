...
public sealed record M18NpcStrikeQueueOptions
{
        ...
        bool enablePermanentSanctions = false)
    {
        ...
        return (mode == M18CandidateMode.ProductionCandidate ? ProductionCandidate : TestLabCandidate) with
        {
            ...
            // ProductionCandidate is an independently observable staged route,
            // not a qualification grant. A config value can request the
            // control, but only the isolated TestLab admission may make this
            // option effective until a separately reviewed production rule
            // exists.
            EnablePermanentSanctions = enablePermanentSanctions && controls && scope == ExecutionScope.TestLab,
        };
    }
    ...
    int MaxTargetType)
{
    ...
    public bool IsButcherPatternBlock => Action == ControlAction.Block && ButcherPatternDetected;
...
public sealed class M18NpcStrikeBehaviorTracker
{
    ...
    private sealed record ControlPoint(long Tick, float X, float Y, bool AlreadyCancelled);
    ...
    private sealed class PendingSequence
    {
        public PendingSequence(int targetSlot, int targetGeneration, int targetType, StrikeSignature signature,
            long tick, ControlPoint? returnPoint, float targetX, float targetY, bool targetPositionComplete,
            bool cancelled)
        {
    ...
    private sealed record CompletedSequence(int TargetSlot, int TargetGeneration, int TargetType,
        StrikeSignature Signature, long Tick, float ReturnX, float ReturnY, bool DeathObserved);
        ...
        M18NpcPlayerControlObservation observation)
    {
        ...
        {
            // A source burst may have been stopped part-way through, but its
            // trailing packet13 restore is still valid sequence evidence. Do
            // not discard that pair merely because an earlier packet28 was
            // canceled; otherwise the next target in the same Butcher loop
            // loses the cross-target continuation signal.
            // The packet13 restore is still sequence evidence when the last
            // packet28 in the source burst was stopped. The adapter passes the
            // cancellation bit for observability, but it must not erase the
            // completed target segment that the next target relies on.
            ...
            bool displacedDeathClose = pending.PostNativeDeathEvidence &&
                pending.StrikeCount >= 2 && pending.TargetPositionComplete &&
                Distance(observation.PositionX, observation.PositionY, pending.TargetX, pending.TargetY) > positionTolerance;
            if (restore || displacedDeathClose)
            {
                // The death bit belongs to the exact completed target segment.
                // It must travel with that event so expiry and FIFO eviction
                // cannot leave a stale account-wide death counter behind.
                state.Completed.Enqueue(new(pending.TargetSlot, pending.TargetGeneration,
                    pending.TargetType, pending.Signature, tick, observation.PositionX,
                    observation.PositionY, pending.PostNativeDeathEvidence));
        ...
        M18NpcStrikeObservation observation)
    {
        ...
        bool preAtTarget = last is not null && Near(last.X, last.Y, observation.TargetX,
        ...
        bool controlJump = previous is not null && last is not null &&
            ...
            pending.TargetType == observation.TargetType)
        {
            ...
            if (sameSignature)
            {
                ...
                return Snapshot(state, auxiliary, true, true,
                    ReturnStable(state, pending.ReturnPoint), repeated, false, repeated,
                    pending.StrikeCount, controlJump);
            }
        ...
        bool returnStable = previous is not null && prior is not null &&
        ...
        state.Pending = new(observation.TargetSlot, observation.TargetGeneration, observation.TargetType, signature,
            tick, previous, observation.TargetX, observation.TargetY,
        ...
        return Snapshot(state, auxiliary, true, true, returnStable, false, crossTarget,
            crossTarget, 1, controlJump, matchingDeathSequences);
    }
    ...
    {
        var state = sessions[slot];
        if (state is null || state.Session != session ||
            state.AccountId > 0 && accountId > 0 && state.AccountId != accountId)
            sessions[slot] = state = new(session, accountId);
        else if (state.AccountId <= 0 && accountId > 0)
...
public sealed class M18NpcStrikeQueue
{
    ...
    public M18NpcStrikeQueueDecision Observe(long tick, M18NpcStrikeObservation observation)
    {
        ...
        if (_options.EnableButcherSequenceStopLoss && behavior.PatternDetected &&
            !observation.TargetFriendly && !observation.TargetDummy)
        {
            bool block = _options.EnablePreForwardBlocks;
            if (block)
                SuppressPendingBehavior(observation.Session);
            // A high value is not illegal merely because it exceeds the
            // current NPC's life.  The first sanction-capable path is the
            // bounded active sequence: a closed, attributed target-death
            // segment followed by a new target with the same strike shape and
            // a fresh packet13 control jump.  The low-damage route retains its
            // historical gate; high damage must additionally be over target
            // maximum life so a normal high-but-valid melee hit remains only a
            // stop-loss observation.
            bool lowSequenceProof = observation.ReceiverDamage > 0 &&
                observation.ReceiverDamage <= _options.LowDamageMaximum &&
                behavior.CrossTargetContinuation && behavior.ControlJumpDetected &&
                behavior.MatchingCompletedTargetDeathSequences >= 1;
            bool highSequenceProof = observation.ReceiverDamage > _options.LowDamageMaximum &&
                observation.TargetLifeMax > 0 &&
                observation.ReceiverDamage > observation.TargetLifeMax &&
                behavior.CrossTargetContinuation && behavior.ControlJumpDetected &&
                behavior.MatchingCompletedTargetDeathSequences >= 1;
            bool butcherSequenceSanction = block &&
                observation.AttributionComplete && observation.LegalExceptionsExcluded &&
                (lowSequenceProof || highSequenceProof) &&
                _options.EnablePermanentSanctions;
            var sequenceDecision = state.Decision(
                Action: block ? ControlAction.Block : ControlAction.Unknown,
                Verdict: butcherSequenceSanction ? Verdict.ProvenCheat :
                    block ? Verdict.UnsafeInput : Verdict.Unknown,
                Reason: butcherSequenceSanction ? "npc-strike-butcher-sequence-account-sanction" :
                    block ? "npc-strike-ordinary-butcher-sequence-preforward-stop" :
                    "npc-strike-ordinary-butcher-sequence-observed-block-disabled",
                counted: false,
                lowDamage: observation.ReceiverDamage <= _options.LowDamageMaximum,
                extremeDamage: false,
                capacityExhausted: false,
                targetSlot: observation.TargetSlot,
                targetGeneration: observation.TargetGeneration) with
            {
                PermanentSanctionCandidate = butcherSequenceSanction,
                ButcherSequenceSanctionCandidate = butcherSequenceSanction,
            };
            return Publish(observation, EnrichDecision(sequenceDecision, observation, stageThreshold,
                extremeGate, behavior));
        }
    ...
    /// and overkill are retained as typed evidence, but neither is sufficient
    /// for an account sanction. The sanction candidate must also have the
    /// bounded active cross-target control sequence already established by the
    ...
    {
        _butcherBehavior.ObservePostNative(tick, observation);
        ...
        bool activeSequenceProof = initial.ButcherPatternDetected &&
            initial.ButcherCrossTargetContinuation &&
            initial.ButcherControlJumpDetected &&
            initial.ButcherMatchingTargetDeathSequences >= 1;
        bool sanction = oneShotDeathEvidence && activeSequenceProof &&
            _options.EnablePermanentSanctions && _options.EnablePreForwardBlocks;
        return initial with
        {
            Action = sanction ? ControlAction.Block : initial.Action,
            Verdict = sanction ? Verdict.ProvenCheat : initial.Verdict,
            Reason = sanction ? "npc-strike-butcher-one-shot-death-post-native-sanction" : initial.Reason,
            PermanentSanctionCandidate = initial.PermanentSanctionCandidate || sanction,
            PostNativeOneShotDeathEvidence = oneShotDeathEvidence,
            PostNativeSanctionCandidate = sanction,
        };
    }
...
public static class M18NpcStrikeQueueRules
{
    ...
    public const string ContractVersion = "terraria1.4.5.8-326-npc-strike-density-butcher-sequence-and-one-shot-sanction-v9";
    ...
    public static BusinessRuleResult Observe(
        ...
        M18NpcStrikeQueueDecision decision)
    {
        bool permanentSanction = decision.PermanentSanctionCandidate &&
            (decision.IsPostNativeSanctionCandidate ||
                decision.IsButcherSequenceSanctionCandidate);
        var action = permanentSanction ? ControlAction.Block : decision.Action;
        var verdict = permanentSanction ? Verdict.ProvenCheat : decision.Verdict;
        var reason = permanentSanction ? decision.IsButcherSequenceSanctionCandidate
            ? "npc-strike-butcher-sequence-account-sanction"
            : "npc-strike-active-sequence-sanction-candidate" : decision.Reason;
        var boundedFacts = BoundedContextFacts(facts)
            ...
            .SetItem("strikeQueueClassification", permanentSanction ?
                decision.IsButcherSequenceSanctionCandidate ? "ordinary-butcher-sequence-account-sanction-candidate" :
                "active-sequence-sanction-candidate" :
                decision.ButcherPatternDetected ? "ordinary-butcher-sequence-preforward-stop" :
            ...
            .SetItem("strikeQueueActionContract", permanentSanction ?
                decision.IsButcherSequenceSanctionCandidate ? "account-sanction-after-complete-butcher-sequence" :
                decision.PostNativeSanctionCandidate ? "account-sanction-after-native-relay-and-active-sequence" :
                    "block-before-native-receiver-active-sequence-sanction-candidate" :
                decision.IsButcherPatternBlock ? "block-before-native-receiver-no-sanction" :
        ...
        return new BusinessRuleResult(
            ...
            verdict,
            reason,
            PredicateSatisfied: permanentSanction,
            PrerequisitesComplete: permanentSanction,
            boundedFacts);
    }
    ...
    public static BusinessRuleResult PostNativeObserve(
        ...
        M18NpcStrikeQueueDecision decision)
    {
        bool permanentSanction = decision.IsPostNativeSanctionCandidate;
        ...
        var boundedFacts = BoundedContextFacts(facts)
            ...
            .SetItem("strikeQueueSamples", BoundedFactValue(FormatSamples(decision)))
            .SetItem("strikeQueueClassification", permanentSanction ?
                "butcher-one-shot-death-active-sequence-post-native-sanction" : "post-native-completion-observed")
            .SetItem("strikeQueueActionContract", permanentSanction ?
                "account-sanction-after-native-relay-and-active-sequence" : "post-native-observation-no-sanction");

        return new BusinessRuleResult(
            RuleId,
            Version,
            permanentSanction ? ControlAction.Block : ControlAction.Unknown,
            permanentSanction ? Verdict.ProvenCheat : Verdict.Unknown,
            permanentSanction ? "npc-strike-butcher-one-shot-death-active-sequence-post-native-sanction" :
