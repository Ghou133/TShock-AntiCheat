...
/// WipeGroundItems sequence can additionally become an account-sanction
/// candidate; ordinary pickups remain Pass and incomplete context remains
/// Unknown.
/// </summary>
...
public sealed class M18GroundItemClearSequenceTracker
{
    ...
    private sealed record PendingClear(int TargetSlot, int TargetGeneration, long Tick,
        ControlPoint? ReturnPoint, float TargetX, float TargetY, byte PacketId, bool AlreadyCancelled);
    ...
    public void ObserveControl(long tick, M18GroundItemPlayerControlObservation observation)
    {
        ...
        {
            bool restore = !observation.AlreadyCancelled && !pending.AlreadyCancelled &&
                pending.ReturnPoint is { } returnPoint &&
                Near(observation.PositionX, observation.PositionY, returnPoint.X, returnPoint.Y, restoreTolerance);
            if (restore)
            {
                state.Completed.Enqueue(new(pending.TargetSlot, pending.TargetGeneration, tick,
                    observation.PositionX, observation.PositionY, pending.TargetX, pending.TargetY,
                    pending.PacketId));
        ...
        M18GroundItemClearObservation observation)
    {
        ...
        bool preAtTarget = last is not null && observation.TargetSnapshotComplete && sequencePositionFinite &&
        ...
        bool jump = previous is not null && last is not null &&
            Distance(previous.X, previous.Y, last.X, last.Y) > restoreTolerance;
        var prior = state.Completed.LastOrDefault();
        bool continuation = prior is not null && previous is not null &&
        ...
        if (!observation.AlreadyCancelled)
            state.Pending = new(observation.TargetSlot, observation.TargetGeneration, tick, previous,
                observation.TargetX, observation.TargetY, observation.PacketId, false);
...
/// server item. A request whose
/// coordinates do not match that item is the exact remote-clear shape exposed
/// by WipeGroundItems and is stopped before the native receiver in candidate
/// mode. No account identity is retained as sanction evidence.
/// </summary>
public sealed class M18GroundItemClearQueue
{
    ...
    public M18GroundItemClearQueueDecision Observe(long tick, M18GroundItemClearObservation observation)
    {
        ...
        bool syncItemDespawn = observation.PacketId == 151;
        bool identityComplete = observation.AccountId > 0 && observation.AttributionComplete;
        bool stopLoss = wipe || remoteOrMismatched;
        bool block = stopLoss && _options.EnablePreForwardBlocks;
        string reason = wipe ? "ground-item-clear-wipe-sequence-stop-loss" :
            remoteOrMismatched ? "ground-item-clear-target-mismatch-stop-loss" :
            !identityComplete ? "ground-item-clear-identity-incomplete" :
            "normal-ground-item-pickup-observed";
        // Keep a WipeGroundItems pending pair even when the first item is
        // stopped. Its following restore and next target are the bounded
        // same-button evidence used for account attribution; an ordinary
        // mismatch without a Wipe sequence still clears the pending state.
        if (stopLoss && block && !wipe && !syncItemDespawn)
            _sequence.CancelPendingClear(observation.Session);
        if (stopLoss && !block)
    ...
    private sealed class SessionState
    {
        ...
        {
            bool block = stopLoss && enablePreForwardBlocks;
            bool normal = !stopLoss && identityComplete;
            // Packet151 carries no client coordinates. The exact WipeGroundItems
            // control/clear/restore sequence is the illegal-source proof here;
            // requiring an independent remote-position mismatch would reject the
            // real button when its preceding packet13 has already moved the
            // accepted server position to the target. Normal pickup and handoff
            // never set WipeSequenceDetected, so they remain outside this path.
            bool sanction = enablePermanentSanctions && block && identityComplete &&
                observation.PacketId == 151 && observation.TargetSnapshotComplete &&
                observation.TargetGeneration > 0 && observation.TargetType > 0 &&
                observation.TargetStack > 0 && observation.ActorPositionSnapshotComplete &&
                sequence.WipeSequenceDetected &&
                (sequence.BatchContinuationDetected || sequence.ControlJumpDetected) &&
                Packet151DistinctTargetCount >= 3;
            return new(
                Enabled: true,
                Action: block ? ControlAction.Block : normal ? ControlAction.Pass : ControlAction.Unknown,
                Verdict: sanction ? Verdict.ProvenCheat :
                    block ? Verdict.ResourceAbuse : normal ? Verdict.Pass : Verdict.Unknown,
                Reason: reason,
                Counted: true,
                ...
                EventsRetained: _count)
            {
                ...
                PermanentSanctionCandidate = sanction,
                WirePacketId = observation.PacketId,
                TargetX = observation.TargetX,
                TargetY = observation.TargetY,
                RequestX = observation.RequestX,
...
public static class M18GroundItemClearQueueRules
{
    ...
    public const string ContractVersion = "terraria1.4.5.8-326-ground-item-clear-sync-item-despawn-sequence-sanction-v6";

    public static BusinessRuleResult Observe(RuleInputContext input,
        M18GroundItemClearQueueDecision decision)
    {
        bool permanentSanction = decision.IsSanctionCandidate;
        var facts = ImmutableDictionary<string, string>.Empty
            ...
            .Add("actionContract", permanentSanction ?
                "block-before-native-receiver-third-current-session-packet151-sanction-candidate" :
                decision.IsResourceBlock ?
                "block-before-native-receiver-connection-stop-loss-no-account-sanction" :
                decision.Action == ControlAction.Pass ? "normal-pickup-native-path" : "record-only")
            .Add("sanctionContract", permanentSanction ?
                "account-sanction-after-third-packet151-in-active-wipe-sequence-with-control-jump-and-complete-server-item-context" : "none");
        return new BusinessRuleResult(RuleId, Version,
            permanentSanction ? ControlAction.Block : decision.Action,
            permanentSanction ? Verdict.ProvenCheat : decision.Verdict,
            permanentSanction ? "ground-item-clear-wipe-sequence-account-sanction" : decision.Reason,
            PredicateSatisfied: permanentSanction,
            PrerequisitesComplete: permanentSanction, facts);
    }
}
