...
public sealed partial class M7NpcStrikeCauseContexts : IDisposable
{
    ...
    public M7NpcStrikeCauseContexts(string fingerprint, M18NpcStrikeQueueOptions? queueOptions = null,
        M18RewardObservationOptions? rewardObservationOptions = null,
        IReadOnlyDictionary<int, string>? importantItemDefinitions = null)
    {
        ...
        importantItemRewards = new(rewardObservationOptions ?? M18RewardObservationOptions.Disabled,
        ...
        bool alreadyCancelled)
    {
        ...
        bool attributed = actor.IsLoggedIn && actor.Account is not null &&
            actor.Account.ID > 0 && actor.Group is not null && actor.Index == session.Slot;
        var serverActor = actor.TPlayer;
        bool serverPositionComplete = serverActor is { active: true } &&
            (uint)session.Slot < Main.player.Length &&
            ReferenceEquals(Main.player[session.Slot], serverActor) &&
            float.IsFinite(serverActor.position.X) && float.IsFinite(serverActor.position.Y);
        ...
        {
            ServerActorX = serverPositionComplete ? serverActor!.position.X : 0,
            ServerActorY = serverPositionComplete ? serverActor!.position.Y : 0,
            ServerActorPositionSnapshotComplete = serverPositionComplete,
        });
    }
    ...
    public BusinessRuleResult Enrich(BusinessRuleResult result, NpcStrikeObservation request, SessionKey session,
        TSPlayer actor, bool alreadyCancelled = false)
    {
        if (alreadyCancelled)
        {
            CancelPendingClientStrike(session);
        ...
        bool actorIdentityComplete = actor.IsLoggedIn && actor.Account is not null &&
            actor.Group is not null && actor.Index == session.Slot;
            ...
            LegalExceptionsExcluded: legalExceptionsExcluded)
        {
            ...
            ActorX = actor.TPlayer?.position.X ?? 0,
            ActorY = actor.TPlayer?.position.Y ?? 0,
            ActorPositionSnapshotComplete = actor.TPlayer is { active: true } player &&
                (uint)session.Slot < Main.player.Length &&
